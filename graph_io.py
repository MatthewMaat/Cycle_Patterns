...
from graph import MyGraph, Edge

DEFAULT_COLOR_SCHEME = "paired12"
NUM_COLORS = 12
LIST_OF_COLORS = ['"grey"','"black"','"red"','"grey"']
LIST_OF_PENWIDTHS = [1,3,3,1]
...
def read_graph(graphclass, f: IO[str]) -> Tuple[MyGraph, List[str], bool]:
    """
...
def read_graph_list(graph_class, f: IO[str]) -> Tuple[List[MyGraph], List[str]]:
    """
...
def load_graph(f: IO[str], graph_class=MyGraph, read_list: bool = False) -> Union[Tuple[List[MyGraph], List[str]], MyGraph]:
    """
...
def input_graph(graph_class=MyGraph, read_list: bool = False) -> Union[Tuple[List[MyGraph], List[str]], MyGraph]:
    """
...
def write_graph_list(graph_list: List[MyGraph], f: IO[str], options=[]):
    """
...
def save_graph(graph_list: Union[MyGraph, List[MyGraph]], f: IO[str], options=[]):
    """
...
def print_graph(graph_list: Union[MyGraph, List[MyGraph]], options=[]):
    """
...
def write_dot(graph: MyGraph, f: IO[str], directed=False, line_ends=True):
    """
    ...
    if line_ends:
        ...
        le = ''
    if directed:
        f.write('digraph G {'+le)
    else:
        f.write('graph G {'+le)

    if graph.label != None:
        f.write('    label ="'+graph.label +'"'+le)
    ...
    for v in graph:
        ...
        elif hasattr(v, 'attractor'):
            if v.attractor == 2:
                options += 'style="filled", fillcolor="tomato",'
            elif v.attractor == -1:
                options += 'style="filled", fillcolor="skyblue",'
            elif v.attractor == 1:
                options += 'style="filled", fillcolor="lightsalmon",'
            elif v.attractor == -2:
                options += 'style="filled", fillcolor="royalblue",'
        if len(options) > 0:
            f.write('    ' + str(name[v]) + ' [' + options[:-1] + ']'+le)
        else:
            f.write('    ' + str(name[v]) + ''+le)
    f.write(le)

    for e in graph.edges:
        options = 'penwidth='+str(LIST_OF_PENWIDTHS[e.in_strategy])+','
        if hasattr(e, 'phiweight'):
            if e.weight != None:
                options += 'label="' + str(e.phiweight) + '",'
        elif hasattr(e, 'weight'):
            if e.weight != None:
                options += 'label="' + str(e.weight) + '",'
        options += 'color=' + LIST_OF_COLORS[e.in_strategy] +','
        ...
        if len(options) > 0:
            options = ' [' + options[:-1] + ']'
        if directed:
            f.write('    ' + str(name[e.tail]) + ' -> ' + str(name[e.head]) + options + le)
        else:
            f.write('    ' + str(name[e.tail]) + '--' + str(name[e.head]) + options + le)
